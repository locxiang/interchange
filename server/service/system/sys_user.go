...
//@author: [piexlmax](https://github.com/piexlmax)
//@function: GetKehuInfoList
//@description: 分页获取客户数据
//@param: info request.PageInfo
//@return: err error, list interface{}, total int64

func (userService *UserService) GetKehuInfoList(info request.PageInfo) (err error, list interface{}, total int64) {
	limit := info.PageSize
	offset := info.PageSize * (info.Page - 1)
	db := global.GVA_DB.Model(&system.SysUser{}).Where("authority_id = ?", 555)
	var userList []system.SysUser
	err = db.Count(&total).Error
	if err != nil {
		return
